...
open_pos = False
def history(length,koef1,koef2,koef3,df):
    sizep = 10
    buys1 = []
    buys2 = []
    buys3 = []
   ...
   # sellorder = []
    ...
    tableafterrequest=basicbacktestATR_MACD.indicator(df,length,koef1,koef2,koef3)
    ...
    if tableafterrequest.empty:
        ...
        #print(table600.ATR[-1])
        for i in range(len(table600)):
            
            if ((table600.ATR.iloc[i])/2)<table600.HIST.iloc[i]:
            ...
            if table600.Low.iloc[i] < table600.LowerStop.iloc[i] and (open_posL1 or open_posL2 or open_posL3):
                if open_posL1:
            ...
            if table600.High.iloc[i] > table600.UpperStop.iloc[i] and (open_posU1 or open_posU2 or open_posU3):   
                if open_posU1:
                ...
                if open_posU3:
                    ...
                    takeU3.append(i)
                needback = True
            if (table600.MA.iloc[i]<table600.High.iloc[i]) or (table600.MA.iloc[i]>table600.Low.iloc[i]):
                needback = False

            if open_posL1 == False:
                if (table600.Lower1.iloc[i]>table600.Low.iloc[i]) and (TrigtonotLower1 == False) and (needback == False):
                    table600.Buy_Signal1.iloc[i] = True
                    open_posL1=True
                    buys1.append(i)
                    signals=signals+1
            elif open_posL1 == True:
                if table600.MA.iloc[i]*0.997<table600.High.iloc[i]:
                    table600.Buy_Signal1.iloc[i] = False
                    table600.Take_Signal.iloc[i] = True
                    open_posL1=False
            ...
            if open_posL2 == False:
                if table600.Lower2.iloc[i]>table600.Low.iloc[i] and (needback == False):
            ...
            if open_posL3 == False:
                if table600.Lower3.iloc[i]>table600.Low.iloc[i] and (needback == False):
    ...
    #--------------------------------------------------------------------------------------#
            ...
            if open_posU1 == False:
                if (table600.Upper1.iloc[i]<table600.High.iloc[i]) and ((TrigtonotUpper1 == False)) and (needback == False):
                    table600.Sell_Signal1.iloc[i] = True
                    open_posU1=True
                    sells1.append(i)
                    signals=signals+1
            elif open_posU1 == True:
                if table600.MA.iloc[i]*1.003>table600.Low.iloc[i]:
                    table600.Sell_Signal1.iloc[i] = False
                    table600.Take_Signal.iloc[i] = True
                    open_posU1=False
            ...
            if open_posU2 == False:
                if table600.Upper2.iloc[i]<table600.High.iloc[i] and (needback == False):
            ...
            if open_posU3 == False:
                if table600.Upper3.iloc[i]<table600.High.iloc[i] and (needback == False):
                ...
                if table600.MA.iloc[i]*1.003>table600.Low.iloc[i]:
                    ...
                    takeU3.append(i)
            if open_posU1 == True and (table600.ATR100.iloc[i]<table600.HIST.iloc[i]):
                    table600.Sell_Signal1.iloc[i] = False
                    table600.Take_Signal.iloc[i] = True
                    open_posU1=False
                    #sellorder.append([[sells1[-1]],[i]])
                    takeU1.append(i)
            if open_posL1 == True and (-(table600.ATR100.iloc[i])>table600.HIST.iloc[i]):
                    table600.Buy_Signal1.iloc[i] = False
                    table600.Take_Signal.iloc[i] = True
                    open_posL1=False
                    #buyorder.append([[buys1[-1]],[i]])
    ...
    # print("sellorders:", sellorder)  
        ...
        #print(signals)
        #print("Расчет..")
        profit1 = pd.concat([table600.Lower1[buys1], 0.997*table600.MA[takeL1]],axis=1)
        #profit1.to_excel("Lower1.xlsx")
        profit2 = pd.concat([table600.Lower2[buys2], 0.997*table600.MA[takeL2]],axis=1)
        #profit2.to_excel("Lower2.xlsx")
        profit3 = pd.concat([table600.Lower3[buys3], 0.997*table600.MA[takeL3]],axis=1)
        #profit3.to_excel("Lower3.xlsx")
        profit4 = pd.concat([table600.Upper1[sells1], 1.003*table600.MA[takeU1]],axis=1)
        #profit4.to_excel("Upper1.xlsx")
        profit5 = pd.concat([table600.Upper2[sells2], 1.003*table600.MA[takeU2]],axis=1)
        #profit5.to_excel("Upper2.xlsx")
        profit6 = pd.concat([table600.Upper3[sells3], 1.003*table600.MA[takeU3]],axis=1)
    ...
    # print("Profit6")
        ...
        totalprofitallwithkoef = totalprofitall*koef - (signals * 0.03)
        #print("Профит")
        ...
        plt.show()
        '''
        return totalprofitallwithkoef,profitfactor,signals,tableafterrequest
