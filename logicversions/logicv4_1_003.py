...
def history(symbol,datestart,dateend):
    ...
    for i in range(len(table600)):
        ...
        if open_posL1 == False:
            ...
            if table600.MA[i]*0.997<table600.High[i]:
...
#--------------------------------------------------------------------------------------#
        if open_posU1 == False:
            ...
            if table600.MA[i]*1.003>table600.Low[i]:
    ...
    #print(signals)
    profit1 = pd.concat([table600.Lower1[buys1], table600.MA[takeL1]*0.997],axis=1)
    #profit1.to_excel("Lower1.xlsx")
    profit2 = pd.concat([table600.Lower2[buys2], table600.MA[takeL2]*0.997],axis=1)
    #profit2.to_excel("Lower2.xlsx")
    profit3 = pd.concat([table600.Lower3[buys3], table600.MA[takeL3]*0.997],axis=1)
    #profit3.to_excel("Lower3.xlsx")
    profit4 = pd.concat([table600.Upper1[sells1], table600.MA[takeU1]*1.003],axis=1)
    #profit4.to_excel("Upper1.xlsx")
    profit5 = pd.concat([table600.Upper2[sells2], table600.MA[takeU2]*1.003],axis=1)
    #profit5.to_excel("Upper2.xlsx")
    profit6 = pd.concat([table600.Upper3[sells3], table600.MA[takeU3]*1.003],axis=1)
