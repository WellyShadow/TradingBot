...
client = Client(config3.api_key,config3.secret)
def klines(symbol):
    try:
    ...
    df = df.astype(float)
    e = talib.EMA(df.Close,18)
    s = talib.SMA(df.Close,18)
    w = talib.WMA(df.Close,18)
    ...
    #print(df.ATR100[-1])
    #print(df.ATR[-1])
    rangema=talib.ATR(df.High,df.Low,df.Close,18) 
    upper1=ma+rangema*4.9
    lower1=ma-rangema*4.9
    upper2=ma+rangema*6.7
    lower2=ma-rangema*6.7
    upper3=ma+rangema*8.5
    lower3=ma-rangema*8.5
    df['Upper1'] = upper1
    df['Lower1'] = lower1
    df['Upper2'] = upper2
    df['Lower2'] = lower2
...
def strategy(symbol,qty) :
    ...
    open_posL1 = False#Lower1
    open_posL2 = False#Lower2
    open_posL3 = False#Lower3
    open_posU1 = False#Upper1
    open_posU2 = False#Upper2
    open_posU3 = False#Upper3
    while True:
        time.sleep(1) 
        df = klines(symbol)
